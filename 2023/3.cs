...
public class _3 { 
    ...
    public static void Run()
    {
        ...
        for (var i = 0; i < list.Count; i++)
        {
            ...
            // if (buffer.Count > 0)
            // {
            //     numbers.Add((int.Parse(new string(buffer.ToArray())), bufferIncludeFlag));
            //     buffer.Clear();
            //     bufferIncludeFlag = false;
            // }
            ...
            for (var j = 0; j < list[0].Count; j++)
            {
                ...
                // analyze if current buffer's content should be included
                if (bufferIncludeFlag == (0, 0))
                {
                    // LEFT from current
                    var column = j - 1;
                    if (j > 0)
                    {
                        if (i > 0 && charIsSymbol(list[i - 1][column]))
                        {
                            bufferIncludeFlag = (i - 1, column);
                        }

                        if (charIsSymbol(list[i][column]))
                        {
                            bufferIncludeFlag = (i, column);
                        }

                        if (i + 1 < list.Count && charIsSymbol(list[i + 1][column]))
                        {
                            bufferIncludeFlag = (i + 1, column);
                        }
                    }
        ...
        Console.WriteLine(numbers
            ...
            .GroupBy(x => x.Item2)
            .Where(x => x.Count() == 2)
            .Select(x => x.ElementAt(0).Item1 * x.ElementAt(1).Item1)
            .Sum());
    }

    static bool charIsSymbol(char c)
    {
        return c == '*';
    }
}