...
public class _10
{
    ...
    private static char startPipe = '7';
    ...
    public static void Logic()
    {
        ...
        var row = array.Single(x => x.Contains('S'));

        sizeY = array.Count;
        sizeX = array[0].Length;
        
        startY = array.IndexOf(row);
        startX = row.IndexOf('S');
        
        var counter = 0;
        List<(int, int)> history = new() { (startY, startX) };
        ...
        // {
        //     if (history.All(entry => entry.Item1 != i))
        //     {
        //         continue;
        //     }
        //     
        ...
        //     {
        //         var entry = history.SingleOrDefault(entry => entry.Item1 == i && entry.Item2 == j);
        //         Console.Write(entry != default ? array[i][j] : '.');
        //     }
        // }
    }

    // public static void PrintHistoryEntry(List<(int, int)> history, (int, int, (int prevY, int prevX)) historyEntry, ref int c)
    // {
    //     Console.WriteLine($"{c++}. y: {historyEntry.Item1} x: {historyEntry.Item2}");
    //     if (historyEntry.Item3.prevY == -1)
    //     {
    //         return;
    //     }
    //
    //     var prev = history.Single(entry =>
    //         entry.Item1 == historyEntry.Item3.prevY &&
    //         entry.Item2 == historyEntry.Item3.prevX);
    //         
    //     PrintHistoryEntry(history, prev, ref c);
    // }

    public static bool LookAround(char current, int currentY, int currentX, List<(int, int)> history, ref int counter)
    {
        if (current == 'S')
        {
            current = startPipe;
        ...
        if (GoRight(current, currentY, currentX + 1, history, ref counter))
        {
            return true;
        }
        // v
        if (GoDown(current, currentY + 1, currentX, history, ref counter))
        {
            return true;
        }
        // <
        if (GoLeft(current, currentY, currentX - 1, history, ref counter))
        {
            return true;
        }
        // ^
        if (GoUp(current, currentY - 1, currentX, history, ref counter))
        {
            return true;
        }
    ...
    public static bool GoRight(char from, int y, int x, List<(int, int)> history, ref int counter)
    {
        if (from != '-' &&
            from != 'L' &&
            from != 'F')
        ...
        return GoTo(y, x, history, ref counter);
    }

    public static bool GoDown(char from, int y, int x, List<(int, int)> history, ref int counter)
    {
        if (from != '|' &&
            from != '7' &&
            from != 'F')
    ...
    public static bool GoLeft(char from, int y, int x, List<(int, int)> history, ref int counter)
    {
        if (from != '-' &&
            from != 'J' &&
            from != '7')
    ...
    public static bool GoUp(char from, int y, int x, List<(int, int)> history, ref int counter)
    {
        if (from != '|' &&
            from != 'L' &&
            from != 'J')
    ...
    public static bool GoTo(int y, int x, List<(int, int)> history, ref int counter)
    {
        // out of bounds
        ...
        //reached start
        if (history.Count > 2 && x == startX && y == startY)
        {
            return true;
        }

        // check history
        if (history.Any(entry => entry.Item1 == y && entry.Item2 == x))
        {
            return false;
        }
...
//     public static string args = @"..F7.
// .FJ|.
// SJ.L7
// |F--J
// LJ...";

    public static string args = @"F-.FJ7|777FF|-J7-|7.L.7-77--.FJ7F|J.FLJJ7.77F-7--L7-.F-FFL-JJ-JFF-|F|FF7.FF-F.JJ-LF.|.77|7F--L7FL--FFJFF-77F-7FFF-7LJ7F7---L-7-F7J.F|.7F7-77
