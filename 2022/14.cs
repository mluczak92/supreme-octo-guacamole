...
namespace adventofcode2022._2022
{
    internal static class _14
    {
        ...
        public static void Run()
        {
            ...
            var colMin = coords.Min(x => x.Column);
            var colMax = coords.Max(x => x.Column);
            var colDiff = colMax - colMin;
            var rowMax = coords.Max(x => x.Row);

            var collRange = Enumerable.Range(0, colDiff + 1);
            var array = Enumerable.Range(0, rowMax + 1)
                .Select(x => collRange.Select(y => ".")
                .ToArray())
            .ToArray();
            ...
            try
            {
                while (true)
                {
                    ...
                    while (true)
                    {
                        if (array[currentPos.row + 1][currentPos.col] == ".")
                        {
                            currentPos.row++;
                        }
                        else if (array[currentPos.row + 1][currentPos.col - 1] == ".")
                        {
                            currentPos.row++;
                            currentPos.col--;
                        }
                        ...
                        {
                            array[currentPos.row][currentPos.col] = "o";
                            counter++;
                            break;
                        }
                    }
