    ...
    internal static class _15
    {
        ...
        public static void Run()
        {
            ...
            var map = Enumerable.Range(0, ymax + 1)
                .Select(x => Enumerable.Range(0, xdiff + 1)
                    .Select(x => ".")
                    .ToArray())
                .ToArray();
            ...
            for (int i = 0; i < logs.Length; i++)
            {
                ...
                currentLog.Sensor.X += -xmin;
                currentLog.Beacon.X += -xmin;

                map[currentLog.Sensor.Y][currentLog.Sensor.X] = "S";
                map[currentLog.Beacon.Y][currentLog.Beacon.X] = "B";
            }
            ...
            foreach (var log in logs)
            {
                ...
                var verticalDiff = Math.Abs(log.Sensor.Y - log.Beacon.Y);
                var horizontalDiff = Math.Abs(log.Sensor.X - log.Beacon.X);
                var diff = verticalDiff + horizontalDiff + 1;

                //4
                for (int i = 0; i < diff; i++)
                {
                    var currentY = log.Sensor.Y + i;
                    ...
                    {
                        var currentX = log.Sensor.X + j - xmin;
            ...
            Console.WriteLine(map[10].Count(x => x == "#"));
            //DisplayMap(map);
        }
