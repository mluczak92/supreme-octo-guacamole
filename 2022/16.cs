...
namespace adventofcode2022._2022
{
    internal class _16
    {
        ...
        static void FindMostEfficientSimulation()
        {
            ...
            var simulationStep = new SimulationStep(rooms[0], _maxMinutes, preOpened, 0, 0);
            ...
            while (q.Count > 0)
            {
                ...
                foreach (var n in closedNeighbors)
                {
                    ...
                    var sum = newOpened.Sum(x => x.Room.FlowRate * x.Minutes);
                    var expectedSum = newOpened.Sum(x => x.Room.FlowRate * (x.Minutes + currentQItem.Left - toSub));
                    var newStep = new SimulationStep(n.Key.Neighbor, currentQItem.Left - toSub, newOpened, sum, expectedSum);
            ...
            var best = results.OrderByDescending(x => x.ExpectedPressure).First();
            Console.WriteLine($"{best.ExpectedPressure}");
        }

        class SimulationStep
        {
            public SimulationStep(Room room, int left, List<(Room Room, int Minutes)> opened,
                int preasure, int expectedPressure)
            {
                Room = room;
                Left = left;
                Opened = opened;
            ...
            public int Preasure { get; }
            public int ExpectedPressure { get; }
            public IEnumerable<SimulationStep> History { get; }

            public override string ToString()
            {
                return string.Join("-", Opened.Select(x => x.Room));
            }
        }
