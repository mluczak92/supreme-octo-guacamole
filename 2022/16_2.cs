...
namespace adventofcode2022._2022
{
    internal class _16_2
    {
        ...
        static void FindMostEfficientSimulation()
        {
            ...
            var simulationStep = new SimulationStep(rooms[0], _maxMinutes, preOpened, 0, null);

            q.Enqueue(simulationStep);
            while (q.Count > 0)
            {
                var currentQItem = q.Dequeue();
                if (currentQItem.Left <= 0 || currentQItem.Opened.Count == rooms.Length)
                {
                    results.Add(currentQItem);
                    continue;
                }

                if (currentQItem.ToString().EndsWith("AA-DD") && currentQItem.Higher.Value.Key.Item2.ToString() == "JJ")
                {

                }
                else if (currentQItem.ToString().EndsWith("AA-DD-JJ"))
                {

                }
                else if (currentQItem.ToString().EndsWith("AA-DD-JJ-BB"))
                {

                }
                else if (currentQItem.ToString().EndsWith("AA-DD-JJ-BB-HH"))
                {

                }
                else if (currentQItem.ToString().EndsWith("AA-DD-JJ-BB-HH-CC"))
                {
                ...
                //1st step only?
                if (currentQItem.Higher == null)
                {
                    var neighborsCartesianProduct = closedNeighbors.SelectMany(left => closedNeighbors, (x, y) => (You: x, Elephant: y))
                        .Where(x => x.You.Key.Neighbor != x.Elephant.Key.Neighbor)
                        .ToArray();

                    foreach (var cart in neighborsCartesianProduct)
                    {
                        //2 at same time
                        if (cart.You.Value == cart.Elephant.Value)
                        {
                            BothAreSame(q, currentQItem, cart.You, cart.Elephant);
                        }
                        //1 higher
                        else
                        {
                            ...
                            OneIsHigher(q, currentQItem, lower, higher);
                        }
                    }
                }
                //both were same cost
                else if (currentQItem.Higher.Value.Value == 0)
                {
                    var higherNeighbors = paths
                        .Where(x => x.Key.Room == currentQItem.Higher.Value.Key.Item2)
                        .Where(x => !currentQItem.Opened.Any(y => y.Room == x.Key.Neighbor))
                        .ToArray();

                    var cartesianProduct = closedNeighbors.SelectMany(left => higherNeighbors, (x, y) => (You: x, Elephant: y))
                        .Where(x => x.You.Key.Neighbor != x.Elephant.Key.Neighbor)
                        .ToArray();

                    foreach (var neighbor in cartesianProduct)
                    {
                        //2 at same time
                        if (neighbor.You.Value == neighbor.Elephant.Value)
                        {
                            BothAreSame(q, currentQItem, neighbor.You, neighbor.Elephant);
                        }
                        //1 higher
                        else
                        {
                            var lower = neighbor.You.Value < neighbor.Elephant.Value ? neighbor.You : neighbor.Elephant;
                            var higher = neighbor.You.Value > neighbor.Elephant.Value ? neighbor.You : neighbor.Elephant;
                            OneIsHigher(q, currentQItem, lower, higher);
                        }
                    }
                }
                //higher was actually higher
                else
                {
                    //last room
                    if (closedNeighbors.Length == 1 && closedNeighbors[0].Key.Neighbor == currentQItem.Higher.Value.Key.Item2)
                    {
                        var toSub = currentQItem.Higher.Value.Value + 1;
                        ...
                        newOpened.Add((currentQItem.Higher.Value.Key.Item2, currentQItem.Higher.Value.Value));

                        var expectedSum = newOpened.Sum(x =>
                        {
                            var expectedMinsLeft = Math.Max(0, Math.Max(0, x.Minutes + currentQItem.Left - toSub));
                            return x.Room.FlowRate * expectedMinsLeft;
                        });

                        results.Add(new SimulationStep(currentQItem.Higher.Value.Key.Item2, 0, newOpened, expectedSum, null));
                    }
                    else
                    {
                        foreach (var neighbor in closedNeighbors.Where(x => x.Key.Neighbor != currentQItem.Higher.Value.Key.Item2))
                        {
                            if (currentQItem.Higher.Value.Value == neighbor.Value)
                            {
                                BothAreSame(q, currentQItem, neighbor, currentQItem.Higher.Value, 1);
                            }
                            else
                            {
                                var lower = neighbor.Value < currentQItem.Higher.Value.Value ? neighbor : currentQItem.Higher;
                                var higher = neighbor.Value > currentQItem.Higher.Value.Value ? neighbor : currentQItem.Higher;
                                OneIsHigher(q, currentQItem, lower.Value, higher.Value, 1);
                            }
                        }
                    }
                }
            ...
            //  1707: AA-DD-JJ-BB-HH-CC-EE 
            var result = results.OrderByDescending(x => x.ExpectedPressure);
            var best = results.Where(x => x.ToString().Contains("AA-DD-JJ"));
            Console.WriteLine($"{result.First()}");
            Console.WriteLine(sw.Elapsed);
        }

        static void BothAreSame(Queue<SimulationStep> q, SimulationStep currentQItem,
            KeyValuePair<(Room Room, Room Neighbor), int> you, KeyValuePair<(Room Room, Room Neighbor), int> elephant, int extraMins = 0)
        {
            var toSub = you.Value + 1;
            var newOpened = new List<(Room Room, int Minutes)>(currentQItem.Opened);
            for (int i = 0; i < newOpened.Count; i++)
            {
                newOpened[i] = (newOpened[i].Room, newOpened[i].Minutes + toSub);
            ...
            newOpened.Add((you.Key.Neighbor, 0 + extraMins));
            newOpened.Add((elephant.Key.Neighbor, 0 + extraMins));
            ...
            var newStep = new SimulationStep(you.Key.Neighbor, currentQItem.Left - toSub, newOpened, expectedSum, new(elephant.Key, elephant.Value - you.Value));
            q.Enqueue(newStep);
        }

        static void OneIsHigher(Queue<SimulationStep> q, SimulationStep currentQItem,
            KeyValuePair<(Room Room, Room Neighbor), int> lower, KeyValuePair<(Room Room, Room Neighbor), int> higher, int extraMins = 0)
        {
            var toSub = lower.Value + 1;
            ...
            newOpened.Add((lower.Key.Neighbor, 0 + extraMins));
            ...
            var newStep = new SimulationStep(lower.Key.Neighbor, currentQItem.Left - toSub, newOpened, expectedSum, new(higher.Key, higher.Value - lower.Value));
            q.Enqueue(newStep);
        }
        ...
        class SimulationStep
        {
            public SimulationStep(Room room, int left, List<(Room Room, int Minutes)> opened, int expectedPressure,
                KeyValuePair<(Room, Room), int>? higher)
            {
                Room = room;
                Left = left;
                Opened = opened;
            ...
            public KeyValuePair<(Room, Room), int>? Higher { get; }
